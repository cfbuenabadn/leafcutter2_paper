...
def get_var_eqtls(tissue, max_pvals=10000):
    """A null distribution of eQTL nominal p-values for `tissue`.

    Note on the source: QTL_analysis.ipynb declares this as
    `get_var_eqtls(tissue, ...)` but hard-codes Testis in the path

        nom_file = f'../code/results/eqtl/GTEx/Testis/cis_100000/nom/{chrom}.txt.gz'

    so the `tissue` argument was ignored and every panel of sup_fig_lambda drew
    its grey null from Testis. That is corrected here: each panel's null comes
    from that panel's own tissue.
    """
...
def make_sqtl_qq(tissues=None, seed=None):
    ...
    and its p-sQTLs, plus a null drawn from that tissue's eQTL nominal pass.
