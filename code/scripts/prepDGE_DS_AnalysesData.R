...
if (grepl("_confounder/$", dsPrefix)) {
  dsPrefix2 <- sub("_confounder/$", "", dsPrefix)
} else {dsPrefix2 <- dsPrefix}

ds.intron <- fread(glue::glue("{dsPrefix2}/{contrast}/ds_perind_numers.counts.noise_by_intron.gz"))[, 1] 
...
out.f <- glue::glue("{outPrefix}/{contrast}.rds")
saveRDS(rds, out.f)
