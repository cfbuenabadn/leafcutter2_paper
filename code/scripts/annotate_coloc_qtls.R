...
labelIntron_sqtl <- function(df) {
    df <- df[, 
        ...
        .(intron = str_split(phe_id, ":") %>% map_chr(~paste(.x[1], .x[2], .x[3], .x[4], sep=":")),
          # extract cluster id
          cluster = str_split(phe_id, ":") %>% map_chr(~paste(.x[1], .x[4], sep=":")),
          # extract intron type (label)
          itype = str_split(phe_id, ":") %>% map_chr(~paste(.x[5], sep=":"))
    )]
    # get cluster type
...
annotate_perm_sqtls <- function(tissue, pheno, gtf, chrom){
    pfx <- '/project/yangili1/cfbuenabadn/leafcutter2_paper/code/results/coloc/qtls/'
    sqtl.file <- glue::glue(pfx, tissue, '/', pheno, '.PermutationPass/', chrom, '.temp.txt')
...
an.sqtls.list <- lapply(chrom_list, function(chrom) {
  annotate_perm_sqtls(tissue, pheno, gtf, chrom)
})
