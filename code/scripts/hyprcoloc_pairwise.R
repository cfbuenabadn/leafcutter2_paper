...
getSummaryStatsPheno <- function(gwas_loci, perm_list, perm_name, p, gtf){
    ...
    if (length(select_tissues) == 0) {
        ...
        print(select_tissues)
        combined_list <- do.call(c, lapply(names(select_tissues), function(tissue) {
            dof1 <- select_tissues[[tissue]]
            tissue_sig_phe <- perm_list[[tissue]] %>% pull(phe_id) %>% unique()
            print(perm_name)
            getTissueRegionStats(region, gwas_loci, tissue, dof1, pheno=perm_name, gtf=gtf, tissue_sig_phe=tissue_sig_phe)
        }))
...
getTissueRegionSNPs <- function(region, gwas_loci_id, tissue, pheno, tissue_sig_phe){
    ...
    #X <- X[!duplicated(X$variant), ] # changed, important
    print(dim(X))
...
getTissueRegionStats <- function(region, gwas_loci_id, tissue, dof, pheno, gtf, tissue_sig_phe) {
    X_stats <- getTissueRegionSNPs(region, gwas_loci_id, tissue, pheno, tissue_sig_phe)
    n <- dof+2
    X_stats$n <- n
...
if (length(tissues) == 0){                          
    file.create(glue('results/coloc/hyprcoloc_results/tables/temp_pairwise/{gwas_loci}.tsv'))                          
                          
    saveRDS(list(betas = NULL, ses=NULL, traits=NULL, rsid=NULL, hyprcoloc_results=NULL), 
           glue('results/coloc/hyprcoloc_results/pairwise_rds/{gwas_loci}.rds'))   
    } else {
...
results_list <- list() # Initialize a list to store results for all tissues
all_tissue_results <- data.frame() # Initialize a data frame to concatenate all tissue results

for (tissue in names(leafcutter_list)) {
  # Subset leafcutter_list and expression_list to the current tissue
  leafcutter_sublist <- list(tissue = leafcutter_list[[tissue]])
  expression_sublist <- list(tissue = expression_list[[tissue]])
    
  names(leafcutter_sublist) <- c(tissue)
  names(expression_sublist) <- c(tissue)
  
  # Get summary statistics for the current tissue
  X_SummaryStats <- getColocSummaryStats(region, gwas_loci, leafcutter_sublist, expression_sublist, gtf = gtf)
  
  # Check if summary stats are NULL or empty for this tissue
  if (is.null(X_SummaryStats) | (length(X_SummaryStats) == 0)) {
    results_list[[tissue]] <- NULL
    next
  }
  
  # Identify common variants for the current tissue
  common_variants <- Reduce(intersect, lapply(X_SummaryStats, function(df) df$variant))
  
  # Filter each dataset to retain only common variants
  ...
  # Run hyprcoloc for the current tissue
  res <- hyprcoloc(as.matrix(betas_hypr), as.matrix(ses_hypr), trait.names = traits, snp.id = rsid)
  res$results['gwas_trait'] <- gwas_loci
  
  # Store the results for the current tissue
  results_list[[tissue]] <- list(
    betas = betas_hypr,
    ses = ses_hypr,
    traits = traits,
    rsid = rsid,
    hyprcoloc_results = res
  )
  
  # Concatenate results for all tissues
  all_tissue_results <- rbind(all_tissue_results, res$results)
}

# Final processing after iterating over all tissues
if (all(sapply(results_list, is.null))) {
  # If results are NULL for all tissues, create empty files
  file.create(glue('results/coloc/hyprcoloc_results/tables/temp_pairwise/{gwas_loci}.tsv'))
  
  saveRDS(
    list(betas = NULL, ses = NULL, traits = NULL, rsid = NULL, hyprcoloc_results = NULL),
    glue('results/coloc/hyprcoloc_results/pairwise_rds/{gwas_loci}.rds')
  )
} else {
  # Save concatenated results to .tsv file
  write_tsv(all_tissue_results, glue('results/coloc/hyprcoloc_results/tables/temp_pairwise/{gwas_loci}.tsv'))
  
  # Save results for all tissues as an RDS file
  saveRDS(results_list, glue('results/coloc/hyprcoloc_results/pairwise_rds/{gwas_loci}.rds'))
}
}            
